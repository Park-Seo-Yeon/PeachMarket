...
def get_opt(type):
    ...
    parser.add_argument('-j', '--workers', type=int, default=4) #1->4
    ...
    parser.add_argument('--result_dir', type=str,
                        default='data/result', help='save result infos')
...
def test_gmm(opt, test_loader, model, board):
    ...
    name = opt.name
    save_dir = os.path.join(opt.result_dir, name)
    ...
    for step, inputs in enumerate(test_loader.data_loader):
        ...
        save_images(warped_cloth, im_names, "data/warp-cloth")
        save_images(warped_mask * 2 - 1, im_names, "data/warp-mask")
...
def test_tom(opt, test_loader, model, board):
    ...
    base_name = os.path.basename(opt.checkpoint)
    save_dir = os.path.join(opt.result_dir, opt.name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    for step, inputs in enumerate(test_loader.data_loader):
        ...
        global s3
        id = im_names[0]
        s3_put_object(s3, AWS_S3_BUCKET_NAME, "data/result/TOM/try-on/{}".format(id), ("data/result/TOM/try_on/{}").format(id))
